...
_markerPos = getMarkerPos _location;
if (_markerPos isEqualTo [0,0,0]) then {
	format["[PRAE Multispawn] - No respawn marker found for %1.. Sending player to default respawn_%2", _location, (side _player)] remoteExec ["diag_log", 2];
	_player setPos (getMarkerPos (format["respawn_%1",(side _player)]));
} else {
	_player setPos (getMarkerPos _location);
};